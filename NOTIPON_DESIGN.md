...
Notiponは、macOS通知センターの通知を自動的に保存し、検索・管理できるメニューバーアプリです。
...
├── Notipon.xcodeproj/
├── Notipon/
│   ├── NotiponApp.swift          # エントリーポイント
│   ├── Info.plist
│   ├── Notipon.entitlements
│   │
│   ├── Core/
...
- **DB場所**: `~/Library/Application Support/Notipon/notifications.db`
- **スキーマ**:
```sql
...
```bash
# Releaseビルド
xcodebuild -scheme Notipon -configuration Release \
  -derivedDataPath ./build build

# 署名なし配布用パッケージ作成
cd build/Build/Products/Release
zip -r Notipon.zip Notipon.app
```
...
## リポジトリ
https://github.com/Mugendesk/Notipon
